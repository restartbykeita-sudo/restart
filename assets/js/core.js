(() => {
  ...
  const App = {
    ...
    storageKey(name,scope=''){return `trail_${name}${scope?`_${scope}`:''}`},
    rememberOrg(id){if(id)localStorage.setItem(this.storageKey('org'),id);else localStorage.removeItem(this.storageKey('org'))},
    rememberEvent(orgId,eventId){if(eventId){localStorage.setItem(this.storageKey('event'),eventId);if(orgId)localStorage.setItem(this.storageKey('event',orgId),eventId)}},
    savedEvent(orgId){return localStorage.getItem(this.storageKey('event',orgId))||localStorage.getItem(this.storageKey('event'))},
