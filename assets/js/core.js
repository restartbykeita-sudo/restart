(() => {
  ...
  const App = {
    ...
    async loadProfile(retries=3){if(!this.user)return null;for(let i=0;i<retries;i++){const{data,error}=await db.from('profiles').select('*').eq('id',this.user.id).maybeSingle();if(error)throw error;if(data){this.profile=data;return data}await new Promise(r=>setTimeout(r,350))}this.profile={id:this.user.id,display_name:this.user.user_metadata?.display_name||this.user.email,email:this.user.email,platform_role:'USER',approval_status:'PENDING',is_active:false};return this.profile},
